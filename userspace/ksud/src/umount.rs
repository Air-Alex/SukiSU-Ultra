...
pub fn add_umount_path(path: &str, check_mnt: bool, flags: u32) -> Result<()> {
    info!("Adding umount path: {} (check_mnt: {}, flags: {})", path, check_mnt, flags);
...
pub fn save_umount_config(paths: &[UmountPathConfig]) -> Result<()> {
    ...
    let json_data: Vec<serde_json::Value> = paths.iter().map(|p| {
        serde_json::json!({
            "path": p.path,
    ...
    fs::write(UMOUNT_CONFIG_FILE, json)
        .context("Failed to write umount config file")?;
...
pub fn load_umount_config() -> Result<Vec<UmountPathConfig>> {
    ...
    let json = fs::read_to_string(UMOUNT_CONFIG_FILE)
        .context("Failed to read umount config file")?;

    let json_data: Vec<serde_json::Value> = serde_json::from_str(&json)
        .context("Failed to parse umount config file")?;
